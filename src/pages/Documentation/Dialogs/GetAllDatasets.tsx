  ...
  return (
    <div>
      ...
      {project ? (
        ...
        <Typography>
          No active project has been set. Please select one first.
        </Typography>
      )}
    </div>
  );
...
function DatasetInfo(props: {
  dataset: { dataset: InstanceType<typeof LbdDataset>; active: boolean };
}) {
  const [datasets, setDatasets] = useRecoilState(d);
  ...
  function makeLabel() {
    ...
    const creator = extract(dataset.data, dataset.url)[DCTERMS.creator][0][
      "@id"
    ];
