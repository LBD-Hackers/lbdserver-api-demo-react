...
export default function CreateDataset(props) {
  ...
  async function createDataset() {
    try {
      ...
      if (file) {
        const theDistribution = await theDataset.addDistribution(
          file,
          undefined,
          {},
          undefined,
  ...
  async function createEmptyDataset() {
    try {
      ...
      await theDataset.addDistribution(
        Buffer.from(""),
        "text/turtle",
        {},
        undefined,
      ...
      {getDefaultSession().info.isLoggedIn ? (
        <div style={{ marginTop: 30 }}>
          {project ? (
            ...
            <Typography>
              You do not have any projects selected in this application. Please
              activate one via the SDK documentation or via your own app.
            </Typography>
          )}
        </div>
      ) : (
        <div style={{ marginTop: 30 }}>
          <Typography>
            You are not authenticated. You will not be able to upload a dataset.
          </Typography>
        </div>
      )}
    </div>
